window.NSK2App = (() => {
  ...
  async function initLaguppstallningPage() {
    ...
    try {
      ...
      if (!laguppstallningBound) laguppstallningBound = true;
  ...
  function syncGoalieAgainstPlayers() {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      Array.from(sel.options).forEach(opt => {
        if (opt.value && opt.value === goalieId) opt.remove();
      });
      if (currentValue === goalieId) sel.value = "";
    }
  }
  ...
  function buildShiftSchedule({ matchNo, playerIds, playersOnField, periods, periodTime, subTime }) {
    ...
    const stats = {};
    ids.forEach((id, idx) => {
      stats[id] = { totalShifts: 0, starts: 0, rank: idx + 1 };
    });
    ...
    function scoreCandidate(id, currentLine, shiftIndex) {
      const s = stats[id];
      let score = 0;
      score += s.totalShifts * 100;
      if (prevLine.includes(id)) score += 80;
      if (shiftIndex === 0) score += s.starts * 50;
      score += s.rank * 2;
      if (currentLine.length > 0) {
        const testLine = [...currentLine, id];
        score += (lineUseCount[lineKey(testLine)] || 0) * 30;
      }
      return score;
    }
    ...
    for (let shiftIndex = 0; shiftIndex < totalShifts; shiftIndex++) {
      ...
      let nextLine = [];
      let preferredPool = rotated.filter(id => !prevLine.includes(id));
      if (preferredPool.length < onField) {
        const rest = rotated.filter(id => prevLine.includes(id));
        preferredPool = preferredPool.concat(rest);
      }
      ...
      if (nextLine.length < onField) {
        const fallback = rotated.filter(id => !nextLine.includes(id));
        while (nextLine.length < onField && fallback.length) {
          fallback.sort((a, b) => scoreCandidate(a, nextLine, shiftIndex) - scoreCandidate(b, nextLine, shiftIndex));
          nextLine.push(fallback.shift());
        }
      }

      const currentOverlap = overlapCount(prevLine, nextLine);
      if (prevLine.length && currentOverlap > Math.max(1, onField - 2)) {
        const bench = rotated.filter(id => !nextLine.includes(id));
        for (let i = nextLine.length - 1; i >= 0; i--) {
          const replacement = bench.find(id => !prevLine.includes(id));
          if (replacement) {
            nextLine[i] = replacement;
            bench.splice(bench.indexOf(replacement), 1);
            if (overlapCount(prevLine, nextLine) <= Math.max(1, onField - 2)) break;
          }
        }
      }
