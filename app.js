...
window.App = (() => {
  ...
  function buildLineups(players, shiftSeconds, matchIndex){
    ...
    for(let i=0;i<rotations;i++){
      ...
      for(let j=0;j<size;j++){
        names.push(rotated[(i*size+j)%rotated.length]);
      }
      ...
      <div class="match">
        <div class="match-top">
          <div>
            ...
            <div class="small">${m.time||"—"} • ${m.opponent ? "mot "+m.opponent : "mot —"} • ${m.field ? "plan "+m.field : "plan —"}</div>
          </div>
          <div class="btnrow">
        ...
        <div>
          ${(m.lineups||[]).map((l,idx)=>`
            <div class="lineup-row ${l.done ? "done" : ""}">
              <div>#${l.no}</div>
              <div>
                ${(l.players||[]).map(p=>`<span class="pill">${escapeHtml(p)}</span>`).join("")}
              </div>
              <div>
                <button class="ghost" data-action="tick" data-index="${i}" data-lineup="${idx}">${l.done ? "✓" : "○"}</button>
              </div>
            </div>
          `).join("")}
        </div>
  ...
  function autosync(){
    ...
    saveLocal();
    setSync("synkar...");
    DB.upsertPool(state.pool).then(()=>setSync("synkad")).catch(()=>setSync("fel"));
  }
  ...
  function savePool(){
    ...
    autosync();
    el("poolMsg").textContent = "Poolspelet sparat.";
  }
  ...
  async function pullPool(){
    try{
      ...
      subscribeCurrentPool();
      setSync("hämtad");
      el("poolMsg").textContent = "Poolspel hämtat.";
    }catch(e){
      el("poolMsg").textContent = e.message || String(e);
    }
  }
  ...
  function savePlayers(){
    ...
    renderMatches();
    autosync();
    el("playersMsg").textContent = "Spelare sparade och bytesschema byggt.";
  }
