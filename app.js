window.NSK2App = (() => {
  ...
  async function generateLineups(){
    ...
    const skaters = ids.filter(Boolean);
    const perLine = Math.min(5, Math.max(3, Math.ceil(skaters.length / 4)));
    const generated = [];

    for(let i=0;i<4;i++){
      const player_ids = [];
      for(let j=0;j<perLine;j++){
        player_ids.push(skaters[(i + j*4) % skaters.length]);
      }
      generated.push({ lineup_no: i+1, player_ids: [...new Set(player_ids)] });
    }

    await DB.replaceLineups(id, generated);
    txt("lineupMsg", "Bytesschema genererat.");
