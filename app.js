...
window.NSK2App = (() => {
  ...
  function notifyShiftSchemaChanged(poolId, lagNo, matchNo) {
    window.dispatchEvent(new CustomEvent("nsk:shift-updated", {
      detail: { poolId: String(poolId), lagNo: String(lagNo), matchNo: String(matchNo) }
    }));
  }

  async function renderShiftSchemaIfVisible(poolId, lagNo, matchNo) {
    if (byId("shiftAllMatchesWrap")) {
      await renderAllShiftSchemasForLag();
      return;
    }

    if (byId("shiftTableWrap")) {
      const sel = byId("shiftMatch");
      if (sel) sel.value = String(matchNo);
      sessionStorage.setItem("nsk2_lag_nr", String(lagNo));
      sessionStorage.setItem("nsk2_match_no", String(matchNo));
      await renderShiftSchema();
    }
  }
  ...
  async function renderCoachMatchView(poolId, lagNo, matchNo) {
    ...
    if (!rows.length) {
      ...
      setHtml("matchInfo", `<div class="matchvy-info-row">Inget bytesschema för vald match.</div>`);
      return;
    }
    ...
    setText("shiftLabel", `Lag ${lagNo} • Match ${matchNo}`);
    setText("currentShiftMeta", `Pågående byte ${matchCurrentShiftIndex + 1} av ${totalShifts}`);
    setText("nextShiftMeta", next ? `Nästa byte ${matchCurrentShiftIndex + 2} av ${totalShifts}` : "Sista byte klart");
  ...
  async function initMatchvyPage() {
    ...
    async function ensureSchemaForMatch(matchNumber) {
      let rows = await DB.listShiftSchema(poolId, lagNo, String(matchNumber));
      if (rows.length) return rows;

      await regenerateShiftSchemaFor(poolId, lagNo, String(matchNumber));
      rows = await DB.listShiftSchema(poolId, lagNo, String(matchNumber));
      return rows;
    }

    async function goToMatch(matchNumber) {
      matchNo = String(matchNumber);
      sessionStorage.setItem("nsk2_match_no", matchNo);
      matchCurrentShiftIndex = 0;
      await ensureSchemaForMatch(matchNo);
      await renderCoachMatchView(poolId, lagNo, matchNo);
      await renderShiftSchemaIfVisible(poolId, lagNo, matchNo);
    }

    await goToMatch(matchNo);
    ...
    if (nextBtn && !nextBtn.dataset.boundMatchvy) {
      ...
      nextBtn.addEventListener("click", async () => {
        const rows = await ensureSchemaForMatch(matchNo);
        const currentShift = rows[matchCurrentShiftIndex];

        if (currentShift) {
          await DB.setShiftDone(poolId, lagNo, matchNo, currentShift.shift_no, true);
          notifyShiftSchemaChanged(poolId, lagNo, matchNo);
        }

        if (matchCurrentShiftIndex < rows.length - 1) {
          matchCurrentShiftIndex += 1;
          await renderCoachMatchView(poolId, lagNo, matchNo);
          await renderShiftSchemaIfVisible(poolId, lagNo, matchNo);
          return;
        }

        const currentMatchNo = parseInt(matchNo, 10) || 1;
        if (currentMatchNo < maxMatches) {
          await goToMatch(currentMatchNo + 1);
          return;
        }

        await renderCoachMatchView(poolId, lagNo, matchNo);
        setText("nextShiftMeta", "Sista matchen klar");
        await renderShiftSchemaIfVisible(poolId, lagNo, matchNo);
      });
    }

    if (!window.__nskShiftSyncBound) {
      window.__nskShiftSyncBound = true;
      window.addEventListener("nsk:shift-updated", async () => {
        await renderShiftSchemaIfVisible(poolId, lagNo, sessionStorage.getItem("nsk2_match_no") || matchNo);
      });
    }
  }
