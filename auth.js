window.Auth = (() => {
  ...
  let client = null, session = null, role = "coach_full", initPromise = null;
  ...
  function updateBadges() {
    const a = document.getElementById("authBadge"), r = document.getElementById("roleBadge");
    if (a) a.textContent = session ? "Inloggad" : "Ej inloggad";
    if (r) r.textContent = "roll: " + (role || "—");
  }
  ...
  async function ensureClient() {
    ...
    if (!window.supabase || !window.supabase.createClient) throw new Error("Supabase-bibliotek kunde inte laddas.");
      ...
      try {
        ...
        client.auth.onAuthStateChange(async (_event, nextSession) => {
          ...
          if (session) {
            showView("appView");
            setMsg("");
            if (window.App && typeof window.App.onSignedIn === "function") window.App.onSignedIn(session);
          } else {
            showView("loginView");
          }
        });
