window.DB = (() => {
  ...
  async function listPlayers() {
    ...
    const { data, error } = await client.from("nsk_players").select("*").eq("team_id", teamId).order("full_name", { ascending: true });
  ...
  async function addPlayer(name) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_players").insert({ team_id: teamId, full_name: String(name || "").trim() }).select("*").single();
    if (error) throw error;
    return data;
  }
  ...
  async function updatePlayer(id, name) {
    const client = await getClient();
    const { error } = await client.from("nsk_players").update({ full_name: String(name || "").trim() }).eq("id", id);
    if (error) throw error;
    return true;
  }
  ...
  async function listCoaches() {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_coaches").select("*").eq("team_id", teamId).order("full_name", { ascending: true });
    if (error) throw error;
    return data || [];
  }
  ...
  async function addCoach(name) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_coaches").insert({ team_id: teamId, full_name: String(name || "").trim(), role: "Tränare" }).select("*").single();
    if (error) throw error;
    return data;
  }
  ...
  async function updateCoach(id, name) {
    const client = await getClient();
    const { error } = await client.from("nsk_coaches").update({ full_name: String(name || "").trim() }).eq("id", id);
    if (error) throw error;
    return true;
  }
  ...
  async function listPools() {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_pools").select("*").eq("team_id", teamId).order("created_at", { ascending: false });
    if (error) throw error;
    return data || [];
  }
  ...
  async function addPool(payload) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_pools").insert({
      team_id: teamId,
  ...
  async function updatePool(id, payload) {
    const client = await getClient();
    const { data, error } = await client.from("nsk_pools").update(payload).eq("id", id).select("*").single();
    if (error) throw error;
    return data;
  }
  ...
  async function listPoolTeamMatchConfigs(poolId) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_pool_team_matches").select("*").eq("team_id", teamId).eq("pool_id", poolId).order("lag_no", { ascending: true }).order("match_no", { ascending: true });
    if (error) throw error;
    return data || [];
  }
  ...
  async function getPoolTeamMatchConfig(poolId, lagNo, matchNo) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_pool_team_matches").select("*").eq("team_id", teamId).eq("pool_id", poolId).eq("lag_no", parseInt(lagNo, 10)).eq("match_no", parseInt(matchNo, 10)).maybeSingle();
    if (error) throw error;
    return data || null;
  }
  ...
  async function getLineup(matchId) {
    const client = await getClient();
    const { data, error } = await client.from("nsk_pool_team_match_people").select("*").eq("match_id", matchId).order("person_type", { ascending: true }).order("sort_order", { ascending: true });
    if (error) throw error;
    return data || [];
  }
  ...
  async function saveLineup(matchId, players, coaches) {
    ...
    (players || []).forEach((playerId, i) => {
      if (!playerId) return;
      rows.push({ match_id: matchId, person_id: playerId, person_type: "player", sort_order: i + 1 });
    });
    (coaches || []).forEach((coachId, i) => {
      if (!coachId) return;
      rows.push({ match_id: matchId, person_id: coachId, person_type: "coach", sort_order: i + 1 });
    });
  ...
  async function listUsedPlayersInPool(poolId, excludeLagNo) {
    ...
    const filteredMatches = (matches || []).filter(m => String(m.lag_no) !== String(excludeLagNo));
  ...
  async function deleteShiftSchema(poolId, lagNo, matchNo) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { error } = await client.from("nsk_pool_team_shifts").delete().eq("team_id", teamId).eq("pool_id", poolId).eq("lag_no", parseInt(lagNo, 10)).eq("match_no", parseInt(matchNo, 10));
    if (error) throw error;
    return true;
  }
  ...
  async function saveShiftSchema(poolId, lagNo, matchNo, shifts) {
    const client = await getClient();
    const teamId = await getTeamId();
    const existing = await listShiftSchema(poolId, lagNo, matchNo);
    const doneMap = {};
    existing.forEach(r => { doneMap[r.shift_no] = !!r.done; });
  ...
  async function listShiftSchema(poolId, lagNo, matchNo) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_pool_team_shifts").select("*").eq("team_id", teamId).eq("pool_id", poolId).eq("lag_no", parseInt(lagNo, 10)).eq("match_no", parseInt(matchNo, 10)).order("shift_no", { ascending: true });
    if (error) throw error;
    return data || [];
  }
  ...
  async function setShiftDone(poolId, lagNo, matchNo, shiftNo, done) {
    const client = await getClient();
    const teamId = await getTeamId();
    const { error } = await client.from("nsk_pool_team_shifts").update({ done: !!done }).eq("team_id", teamId).eq("pool_id", poolId).eq("lag_no", parseInt(lagNo, 10)).eq("match_no", parseInt(matchNo, 10)).eq("shift_no", parseInt(shiftNo, 10));
    if (error) throw error;
    return true;
  }
  ...
  async function listPlayerCoachMap() {
    const client = await getClient();
    const teamId = await getTeamId();
    const { data, error } = await client.from("nsk_player_coach_map").select("*").eq("team_id", teamId).order("player_name", { ascending: true });
    if (error) throw error;
    return data || [];
  }
